// PERBAIKI IMPORT DENGAN BENAR - GUNAKAN DATA MOCK
...
const apotekController = {
  // API 1: GET RESEP PASIEN BERDASARKAN NO_PASIEN DAN TANGGAL PELAYANAN
  getResepPasien: async (req, res) => {
    try {
      ...
      // Cari data master pasien - DI DATA MOCK FIELDNYA no_pasien
      ...
      // Cari semua registrasi pasien di pasienRalan berdasarkan no_pasien
      ...
      const resepDetail = resepPasien.map(resep => {
        ...
        return {
          ...
          detail_pelayanan: pelayanan ? {
            ...
            jam_periksa: pelayanan.jam_periksa,
          ...
          nama_dokter_peresep: dokter ? dokter.nama_dokter : 'Dokter tidak ditemukan',
          spesialisasi_dokter: dokter ? dokter.spesialisasi : '-',
          nama_dokter_pemeriksa: pelayanan ? masterDokter.find(d => d.kode_dokter === pelayanan.kode_dokter)?.nama_dokter || '-' : '-',
        ...
        data: {
          ...
          resep: resepDetail.map((resep, index) => ({
            ...
            info_dokter_peresep: {
              ...
              nama_dokter: resep.nama_dokter_peresep,
            ...
            info_pelayanan: resep.detail_pelayanan ? {
              ...
              jam_periksa: formatJam(resep.detail_pelayanan.jam_periksa),
              ...
              nama_dokter_periksa: resep.nama_dokter_pemeriksa
            } : { 
              tgl_periksa: '-', 
              diagnosa: '-', 
              keluhan: '-', 
  ...
  // API 3: DETAIL RESEP BY NO_RESEP
  detailResep: async (req, res) => {
    try {
      ...
      if (!resep) {
        console.log(`   ❌ Resep ${no_resep} tidak ditemukan`);
        return res.json({
          success: false,
      ...
      // Cari data dokter peresep
      const dokter = masterDokter.find(d => d.kode_dokter === resep.dokter);
      
      // Cari data dokter pemeriksa
      const dokterPemeriksa = pelayanan ? masterDokter.find(d => d.kode_dokter === pelayanan.kode_dokter) : null;
      ...
      const response = {
        ...
        data: {
          ...
          info_pelayanan: pelayanan ? {
            ...
            dokter_periksa: dokterPemeriksa ? dokterPemeriksa.nama_dokter : 'Tidak diketahui',
  ...
  getObatByNoReg: async (req, res) => {
    try {
      ...
      if (resepList.length === 0) {
        console.log(`   ⚠️  Tidak ada resep untuk registrasi ${no_reg}`);
        return res.json({
          success: true,
